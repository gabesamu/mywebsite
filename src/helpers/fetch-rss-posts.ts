...
import type { RssPost, RssPosts, RssUrlItem, RssUrlList } from '$src/types/RssXml';
...
export const fetchPostsFromRss = (
	...
	svelteFetch?: (input: RequestInfo | URL) => Promise<Response>
) => {
	...
	const removeDuplicates = (postsList: RssPosts): RssPosts => {
		return postsList.filter((post) => postsList.find((p) => p.url === post.url));
	};
	...
	const fetchSingleURL = async (rssUrl: RssUrlItem) => {
		const fetchMethod = svelteFetch || fetch;
		return fetchMethod(rssUrl.url)
			.then((response) => response.text())
			.then((rawXml) => {
				const parsedPosts = parseXml(rawXml);
				return parsedPosts.map((post) => {
					return { ...post, sourceFeed: rssUrl.label };
				});
			});
	};
	...
	const fetchAllPosts = async (feeds: RssUrlList): Promise<RssPost[]> => {
		return await Promise.all(
			...
			feeds.map((feed) => fetchSingleURL(feed))
		).then(
