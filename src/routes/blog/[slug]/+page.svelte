<script lang="ts">
	...
	const getPostToRender = (posts: RssPosts, slug: string): RssPost | undefined => {
	...
	const makeTitle = () => {
		...
		return `${title} | Gabriel's Blog`;
	};

	const makeDescription = () => {
		return postToRender
			? postToRender?.description.replace(/<\/?[^>]+(>|$)/g, '').substring(0, 200) + '...'
			: "Read this article, and many more on Gabriel's blog";
	};
...
{#if postStatus === PostStatus.Ready && postToRender}
	<article>
		...
		<div class="meta">
			<time datetime={postToRender.pubDate} title="First published on: {postToRender.pubDate}">
