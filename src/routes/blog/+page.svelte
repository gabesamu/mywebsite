<script lang="ts">
	...
	import { filtered, searchTerm } from '$src/store/BlogStore';
	...
	const makeHoverText = (post: RssPost) => {
		...
		const timeAgo =
			daysBack <= 365 ? `${daysBack} days ago` : `${Math.round(daysBack / 365)} years ago`;
...
{#if $filtered.length === 0}
	<div class="no-results">
		<h4>No Results :(</h4>
		<p>There were no results returned.</p>
	</div>
