...
const initialTheme =
	(browser ? (localStorage.getItem('theme') as Theme) : null) || config.defaultTheme;
...
export const themeColors = derived([theme], ([$theme]) => {
	...
	return Object.keys(themeData).reduce(
		(acc, key) => {
...
export const encodedThemeCss: Readable<string> = derived([theme], ([$theme]) => {
	let cssProperties = '';
	const themeData = (config.colorSchemes || {})[$theme] || {};
